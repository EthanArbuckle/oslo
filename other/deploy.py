...
BINARY_DEPLOY_INFO = {
    "oslo_ios": BinaryInstallInformation(Path("usr/bin/oslo"), Path("other/entitlements.plist").resolve()),
}
...
def deploy_to_device(local_path: Path, binary_deploy_info: BinaryInstallInformation) -> None:
    ...
    try:
        ...
        logger.warning(f"Failed to delete existing binary on device: {e}")

    try:
        on_device_parent_dir = binary_deploy_info.on_device_path.parent
        run_command_on_device(f"mkdir -p {on_device_parent_dir.as_posix()}")
    except subprocess.CalledProcessError as e:
        logger.warning(f"Failed to create parent directory on device: {e}")
    ...
    root_prefix = determine_jb_root_prefix()
    on_device_ents_path = root_prefix / "tmp/entitlements.plist"
    try:
        ...
        raise DeploymentError(f"Failed to copy entitlements file to device: {e}")
